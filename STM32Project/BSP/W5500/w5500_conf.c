...
uint16 local_port=4000;	                       					/*定义本地端口*/

/*定义远端IP信息*/
uint8  remote_ip[4]={192,168,1,4};											/*远端IP地址*/
uint16 remote_port=5000;																/*远端端口号*/

/*IP配置方法选择，请自行选择*/
uint8	ip_from=IP_FROM_DHCP;//IP_FROM_DEFINE;				
...
void gpio_for_w5500_config(void)
{
	GPIO_InitTypeDef GPIO_InitStructure;
																		 /*初始化STM32 SPI1接口*/
	RCC_APB2PeriphClockCmd(WIZ_SCLK_GPIO_RCC| WIZ_MISO_GPIO_RCC  | WIZ_MOSI_GPIO_RCC | 
	                       WIZ_SCS_GPIO_RCC |WIZ_INT_GPIO_RCC    |WIZ_SCS_GPIO_RCC   |WIZ_RESET_GPIO_RCC, ENABLE);
	
	/* Configure SPIy pins: SCK */
	GPIO_InitStructure.GPIO_Pin   = WIZ_SCLK;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_AF_PP;
	GPIO_Init(WIZ_SCLK_GPIO_PORT, &GPIO_InitStructure);

	/* Configure SPIy pins:MISO */
	GPIO_InitStructure.GPIO_Pin   = WIZ_MISO ;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_AF_PP;
	GPIO_Init(WIZ_MISO_GPIO_PORT, &GPIO_InitStructure);

	/* Configure SPIy pins: MOSI */
	GPIO_InitStructure.GPIO_Pin   = WIZ_MOSI;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_AF_PP;
	GPIO_Init(WIZ_MOSI_GPIO_PORT, &GPIO_InitStructure);

	/* Configure SPIy pins: WIZ_SCS */
	GPIO_InitStructure.GPIO_Pin   = WIZ_SCS;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_Out_PP;
	GPIO_Init(WIZ_SCS_GPIO_PORT, &GPIO_InitStructure);
	GPIO_SetBits(WIZ_SCS_GPIO_PORT, WIZ_SCS);
	
//		/* Configure SPIy pins: WIZ_RESET */
	GPIO_InitStructure.GPIO_Pin   = WIZ_RESET;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_IPU;
	GPIO_Init(WIZ_RESET_GPIO_PORT, &GPIO_InitStructure);
	
		/* Configure SPIy pins: WIZ_INT */
	GPIO_InitStructure.GPIO_Pin   = WIZ_INT;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_IPU;
	GPIO_Init(WIZ_INT_GPIO_PORT, &GPIO_InitStructure);
...
{
  I2C_ReadByte(EEPROM_MSG.mac,EEPROM_MSG_LEN,0);//,EE_DEV_ADDR);
	delay_us(10);
}
